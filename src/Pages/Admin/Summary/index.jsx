...
export const IdeaSummary = () => {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4">Ideas Approved</h1>
      <FilterBySelect filters={filters} onFilterChange={handleFilterChange} />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
        {idea?.map((item, index) => (
          <div key={index} className="bg-white shadow rounded-lg p-6 mb-4 border-t-[3px] border-[#15803D]">
            ...
            <div className="flex items-center justify-between mt-1">
              <p className="text-sm text-gray-600">
                Submitted on {new Date(item?.applicationDate).toLocaleDateString()}
              </p>
            </div>
          </div>
        ))}
